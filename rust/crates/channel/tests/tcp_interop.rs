...
use channel::{ChannelConfig, ChannelCore, LinkRx, LinkTx, TcpConnector};
...
async fn tcp_link_transmit_reaches_peer_and_decodes() {
    ...
    tokio::spawn(channel::tcp_server::serve(listener, core));
    ...
    a_tx.send(ClientMsg::TransmitAudio {
        audio_b64: channel::samples_to_b64(&wave),
    })
    .await
    .unwrap();
    ...
    let samples = channel::b64_to_samples(&audio_b64).unwrap();
...
async fn tcp_second_transmitter_gets_channel_busy() {
    ...
    b_tx.send(ClientMsg::TransmitAudio {
        audio_b64: channel::samples_to_b64(&modem.modulate(b"small", Mode::Bpsk)),
    })
    .await
    .unwrap();
