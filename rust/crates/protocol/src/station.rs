...
use channel::{Connector, LinkRx, LinkTx};
use modem::Modem;
use netproto::{ClientMsg, Frame, Mode, ServerMsg, BLOCK_SIZE};
...
impl<C: Connector> StationShared<C> {
    fn log(&self, msg: impl Into<String>) {
        ...
        let _ = self
            .events
            .send(StationEvent::Log(format!("[{}] {m}", self.callsign)));
    }
    ...
    async fn send_frame(&self, frame: Frame, mode: Mode) -> bool {
        ...
        let audio_b64 = channel::samples_to_b64(&samples);
        ...
        for _ in 0..40 {
            ...
            let reply = {
                ...
                *self.tx_reply.lock().unwrap() = Some(rtx);
                if tx
                    .send(ClientMsg::TransmitAudio {
                        audio_b64: audio_b64.clone(),
                    })
                    .await
                    .is_err()
                {
                ...
                match rx.recv().await {
                    ...
                    Some(msg) => match msg {
                        ...
                        ServerMsg::RxAudio { audio_b64 } => {
                            let Ok(samples) = channel::b64_to_samples(&audio_b64) else {
                                continue;
                            };
    ...
    fn touch_roster(self: &Arc<Self>, callsign: &str) {
        let was_lost = {
            ...
            let was_lost = st
                .roster
                .get(callsign)
                .map(|e| e.status == RosterStatus::Lost)
                .unwrap_or(false);
            st.roster.insert(
                callsign.to_string(),
                RosterEntry {
    ...
    fn age_roster(&self) {
        ...
        let mut changed = false;
        {
            ...
            st.roster
                .retain(|_, info| now.duration_since(info.last_seen) <= remove_to);
    ...
    async fn on_beacon(&self, frame: &Frame) {
        ...
        let mut role_evt: Option<Role> = None;
        {
            ...
            for c in roster {
                if c != &self.callsign {
                    let e = st.roster.entry(c.clone()).or_insert(RosterEntry {
                        last_seen: now,
                        status: RosterStatus::Active,
                    });
                    e.last_seen = now;
    ...
    async fn send_bulk(self: Arc<Self>, path: PathBuf, dst: String) {
        ...
        for round_no in 0..6 {
            let missing = self
                .wait_for_status(&transfer_id, Duration::from_secs(6))
                .await;
            if !self.connected.load(Relaxed) {
                self.log(format!(
                    "[{transfer_id}] the link dropped, the transfer is suspended"
        ...
        for (i, &seq) in seqs.iter().enumerate() {
            ...
            let data = {
                ...
                match st
                    .transfers_out
                    .get(transfer_id)
                    .and_then(|t| t.blocks.get(&seq))
                {
                    Some(d) => d.clone(),
                    None => return false,
                }
            };
            let crc = netproto::crc32(&data);
            let ok = self
                .send_frame(
                    Frame::BulkBlock {
                        src: self.callsign.clone(),
    ...
    fn on_bulk_block(&self, frame: &Frame) {
        ...
        let Ok(bytes) = netproto::b64d(data) else {
            return;
        };
    ...
    async fn on_bulk_end(self: &Arc<Self>, frame: Frame) {
        ...
        if missing.is_empty() {
            ...
            self.log(format!(
                "[{transfer_id}] {}/{total} blok eksik, isteniyor",
                missing.len()
            ));
        }
            ...
            roster: st
                ...
                .map(|(c, e)| {
                    (
                        c.clone(),
                        e.status,
                        now.duration_since(e.last_seen).as_secs_f64(),
                    )
                })
                .collect(),
            transfers_in: st
